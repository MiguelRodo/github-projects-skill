...
add_scope() {
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$repository" "$queue_label" "$route_label" "$sub_label" "$project_identity" "$sub_key" >>"$scopes"
}
...
while IFS=$'\t' read -r repository queue_label route_label sub_label project_identity sub_key; do
  ...
  while IFS=$'\t' read -r number url; do
    [ -n "$number" ] || continue
    printf '%s\t%s\t%s\t%s\t%s\n' \
      "$repository" "$number" "$url" "$project_identity" "$sub_key" >>"$candidates"
  done <<<"$found"
done <"$scopes"
...
printf 'status\tready\n'
while IFS=$'\t' read -r repository number url project_identity sub_key; do
  printf 'candidate\t%s\t%s\t%s\t%s\t%s\n' \
    "$repository" "$number" "$url" "$project_identity" "$sub_key"
done <"$tmp/unique"
