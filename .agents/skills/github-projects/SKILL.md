...
A labelled existing task issue is a valid reconciliation target without a separate authority comment. Its ordinary imperative prose describes the work the task represents and never causes the issue's administrative work to be skipped: apply the bounded administrative instruction it states and leave the substantive work untouched. A temporary handoff or an unusual, explicit mutation still needs the unedited authority comment that the reference describes. Report the mutation as queued, not completed.
